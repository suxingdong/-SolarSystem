...
namespace engine
{
    public class ByteBuffer
    {
        ...
        private static ByteBuffer readBuffer = new ByteBuffer();
        public static byte[] read(byte[] receive)
        {
            readBuffer.reset(receive);
            int length = readBuffer.readInt();
            byte[] result = new byte[length];
            Buffer.BlockCopy(receive, 4, result, 0, length);
            return result;
        }
        ...
        public CMessageID code;//read buffer protocol code
